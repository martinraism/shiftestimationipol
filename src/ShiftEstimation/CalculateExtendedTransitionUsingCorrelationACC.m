function [output] = CalculateExtendedTransitionUsingCorrelationACC(u, v, tol, maxIt)
    ...
    % Define frequencies to use as in the original paper
    frequenciesToUse = [0 1;0 2;1 0; 1 1; 1 2; 2 0; 2 1; 2 2];
    ...
    while (dif > tol && qtyIt < maxIt)
        ...
        output = CalculateExtendedTransitionUsingCorrelationConfigurable(us,vs, frequenciesToUse); 
...
%     if (qtyIt ~= maxIt)
%         fprintf('ACC with tol:%f finished using %d iterations.\n', tol, qtyIt);
%     end
    output = resAcum;
